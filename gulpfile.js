...
gulp.task(`typecheck`, shell.task([
  `${path.join('node_modules', '.bin', 'tsc')} --noemit`,
]))
...
gulp.task(`coverage:run`, shell.task([
  `${path.join('node_modules', '.bin', 'nyc')} --reporter html --reporter lcov --extension .ts --extension .tsx --include src/lib --exclude src/**/*.test.ts --exclude src/**/*.test.tsx --all ${path.join('node_modules', '.bin', 'mocha')}`
]))

gulp.task(`coverage`, gulp.series(`coverage:clean`, `coverage:run`))

gulp.task(`coverage:submit`, function () {
  return gulp.src(path.join('coverage', 'lcov.info')).pipe(coveralls())
})
...
gulp.task(`build:transpile`, shell.task([`${path.join('node_modules', '.bin', 'tsc')} --outDir .`]))
...
gulp.task(`site:ci`, gulp.series(`site`, function (done) {
  pages.clean()
  pages.publish(path.join('docs-site', 'public'), undefined, done)
}))
